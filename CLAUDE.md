...
Key config sections: `experiment`, `mode` (train/generate/select/evaluate), `compute`, `model`, `data`, `output`, `training`, `generation`, `evaluation`, `logging`.
...
- **Selection evaluation report** (`evaluation_report.py`): aggregates `evaluation.json` files from selection-eval experiments into comparison tables (CSV/JSON) across diffusion variants, generation configs, and selection methods. Key metrics: FID, precision, recall for real-vs-selected and real-vs-generated comparisons. Run standalone via `python -m src.experiments.sample_selection.evaluation_report [--output-dir DIR]`
...
- Config: `evaluation.checkpoint` required in evaluate mode, `output.subdirs.reports` required
- Output: `reports/evaluation.json` containing per-class and aggregate metrics
