...
- [ ] Phase 1: Update `default.yaml`
  - [ ] Task 1.1: Remove `interval` key
  - [ ] Task 1.2: Add `log_images_interval`, `log_sample_comparison_interval`, `log_denoising_interval`

- [ ] Phase 2: Add `sample_with_intermediates()` to `DiffusionSampler`
  - [ ] Task 2.1: Implement method that returns `(samples, denoising_sequence)` where `denoising_sequence` is shape `(T, C, H, W)`
  - [ ] Task 2.2: Add unit tests for `sample_with_intermediates()`

- [ ] Phase 3: Update `DiffusionTrainer`
  - [ ] Task 3.1: Replace `sample_images: bool` + `sample_interval: int` constructor params with `log_images_interval`, `log_sample_comparison_interval`, `log_denoising_interval` (all `Optional[int]`)
  - [ ] Task 3.2: Update `_generate_samples()` to generate samples once and call all three methods conditionally
  - [ ] Task 3.3: Update training loop trigger condition in `train()`
  - [ ] Task 3.4: Update class docstring and constructor docstring

- [ ] Phase 4: Update `main.py`
  - [ ] Task 4.1: Read new interval keys from `visualization_config`
  - [ ] Task 4.2: Pass new params to `DiffusionTrainer()`
  - [ ] Task 4.3: Remove `sample_images` and `sample_interval` arguments

- [ ] Phase 5: Update tests
  - [ ] Task 5.1: Update `test_trainer.py` — replace `sample_images`/`sample_interval` with new params
  - [ ] Task 5.2: Add trainer tests: verify each method is called at the right interval
  - [ ] Task 5.3: Add trainer tests: verify samples generated only once when multiple intervals trigger simultaneously
  - [ ] Task 5.4: Update `test_logger.py` — add tests for `log_sample_comparison` and `log_denoising_process` being called
  - [ ] Task 5.5: Update integration tests if needed

- [ ] Phase 6: Validate
  - [ ] Task 6.1: Run full test suite (`pytest`)
