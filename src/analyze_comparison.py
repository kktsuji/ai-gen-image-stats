...
for base_path in sorted(glob(_base_path)):
    ...
    print(
        f"  Min/Max Normal: [{abnormal_normal.min():.2f}%, {abnormal_normal.max():.2f}%]"
    )
    print(
        f"  Min/Max Synth:  [{abnormal_synth.min():.2f}%, {abnormal_synth.max():.2f}%]"
    )
    ...
    for idx, (metric, title) in enumerate(metrics_to_plot):
        ...
        for exp, label, color in [
            ...
            ax.plot(
                grouped.index, grouped["mean"], label=label, color=color, linewidth=2
            )
            ax.fill_between(
                grouped.index,
    ...
    print("\n" + "=" * 80)
    print(
        f"Visualization saved to: {os.path.join(base_path, 'training_comparison.png')}"
    )
    print("=" * 80)
