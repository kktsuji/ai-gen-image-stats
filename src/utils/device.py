...
def get_device(
    ...
    Returns:
        torch.device: The selected device
    """
    # Detect mismatched parameters on cached singleton inside the lock to
    # avoid a TOCTOU race between the check and the reset.
    global _global_device_manager
    with _device_manager_lock:
        needs_reset = False
        ...
        if _global_device_manager is None or needs_reset:
            _global_device_manager = DeviceManager(
                force_cpu=force_cpu, device_id=device_id
            )
